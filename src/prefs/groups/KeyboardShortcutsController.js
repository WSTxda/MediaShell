...
function createActionCopy() {
    return Object.freeze({
        "play-pause": Object.freeze({ title: _("Play / Pause"), section: "playback" }),
        "next-track": Object.freeze({ title: _("Next Track"), section: "playback" }),
        "previous-track": Object.freeze({ title: _("Previous Track"), section: "playback" }),
        ...
        "toggle-shuffle": Object.freeze({ title: _("Shuffle"), section: "playback" }),
        ...
        "open-preferences": Object.freeze({ title: _("Preferences"), section: "interface" }),
...
export default class KeyboardShortcutsController {
    ...
    init() {
        ...
        this.shortcutOverviewButton = this.builder.get_object("btn-interactions-shortcut-overview");
        this.resetShortcutsRow = this.builder.get_object("br-interactions-reset-shortcuts");

        for (const definition of INPUT_ACTION_DEFINITIONS) {
            const row = this.builder.get_object(shortcutRowId(definition.id));
            if (!row) throw new Error(`Shortcut preference row not found: ${definition.id}`);
            this.connectOwnedSignal(row, "activated", () => this.presentShortcutEditor(definition));
        }

        this.connectOwnedSignal(this.shortcutOverviewButton, "clicked", () => this.presentShortcutsOverview());
        this.connectOwnedSignal(this.resetShortcutsRow, "activated", () => this.presentResetShortcutsConfirmation());
    }
    ...
    presentShortcutEditor(definition) {
        ...
        session.keyPressedSignalId = keyController.connect("key-pressed", (_controller, keyval, keycode, state) =>
    ...
    handleShortcutKeyPressed(session, keyval, keycode, state) {
        ...
        if (isValidBinding(mask, keycode, keyval) && isValidAccelerator(mask, keyval)) {
            session.shortcutLabel.accelerator = Gtk.accelerator_name_with_keycode(null, keyval, keycode, mask);
        }
        return Gdk.EVENT_STOP;
    }
    ...
    presentShortcutsOverview() {
        ...
        for (const sectionId of SECTION_ORDER) {
            const group = new Adw.PreferencesGroup({ title: this.sectionCopy[sectionId] });
            for (const definition of INPUT_ACTION_DEFINITIONS.filter(
                ({ id }) => this.actionCopy[id].section === sectionId,
            )) {
                const accelerator = this.settings.get_strv(definition.shortcutKey)[0] ?? "";
    ...
    presentResetShortcutsConfirmation(parent = this.preferencesWindow) {
        ...
        dialog.connect("response", (_dialog, response) => {
            ...
            if (this.resetConfirmationDialog === dialog) this.resetConfirmationDialog = null;
        });
        dialog.present(parent);
    }

    resetKeyboardShortcuts() {
        for (const shortcutKey of KEYBOARD_SHORTCUT_KEYS) this.settings.reset(shortcutKey);
        for (const shortcutLabel of this.overviewShortcutLabels.values()) shortcutLabel.accelerator = "";
        this.preferencesWindow.add_toast(
            new Adw.Toast({
