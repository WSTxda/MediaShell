...
class BlockedAppChooserDialog extends Adw.Dialog {
  _init(params = {}) {
    ...
    this.searchButton.connect("toggled", () =>
      this.setSearchMode(this.searchButton.active),
    );
    this.searchBar.connect("notify::search-mode-enabled", () => {
      const searchModeEnabled = this.searchBar.search_mode_enabled;
      if (this.searchButton.active !== searchModeEnabled)
        this.searchButton.active = searchModeEnabled;
    ...
    this.listBox.connect("row-activated", (_listBox, row) =>
    ...
    this.appInfoMonitorSignalId = this.appInfoMonitor.connect("changed", () =>
      this.scheduleAppsRefresh(),
    );
  ...
  populateApps() {
    const selectedAppId =
      this.getAppIdFromRow(this.listBox.get_selected_row()) || null;
    ...
    for (const app of installedApps) {
      ...
      this.searchIndexByRow.set(
        row,
        buildSearchIndex([appName, appId, ...getAppSearchAliases(app)]),
      );
    ...
    if (selectedRow && this.matchesSearch(selectedRow))
      this.listBox.select_row(selectedRow);
  ...
  getActionRow(row) {
    if (!row) return null;
    if (typeof row.title === "string" || typeof row.subtitle === "string")
      return row;
  ...
  getAppIdFromRow(row) {
    if (!row) return "";
    return (
      this.appIdByRow.get(row) ?? String(this.getActionRow(row)?.subtitle ?? "")
    );
  }
