 ...
 * fallback policy identical for Popup and Top Bar metadata without requiring
 ...
 */
export function buildTrackInformationText(metadata = {}, fields = [], fallbacks = {}) {
    const trackInformation = readTrackInformation(metadata, fallbacks);
    const values = [];

    for (const field of fields) {
        const fieldValue = TrackInformationFields[field];
        if (fieldValue === TrackInformationFields.TITLE) values.push(trackInformation.title);
        else if (fieldValue === TrackInformationFields.ARTIST) values.push(trackInformation.artist);
        else if (fieldValue === TrackInformationFields.ALBUM) values.push(trackInformation.album);
        else if (fieldValue === TrackInformationFields.DISC_NUMBER) values.push(trackInformation.discNumber);
        else if (fieldValue === TrackInformationFields.TRACK_NUMBER) values.push(trackInformation.trackNumber);
        else values.push(field);
    }

    return values.filter((value) => value != null).join(" ").replace(/[\r\n]+/g, " ");
}
