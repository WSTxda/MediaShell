...
export default class TopBarPointerHandler {
    ...
    install() {
        ...
        if (this.topBarButton._clickGesture && typeof this.topBarButton._clickGesture.set_enabled === "function")
            this.topBarButton._clickGesture.set_enabled(false);

        for (const actor of [this.topBarButton.topBarActionBoxBefore, this.topBarButton.topBarActionBoxAfter])
            this.#installForActor(actor);
    }

    #installForActor(actor) {
        if (typeof Clutter.ClickGesture !== "undefined") {
            // GNOME 49+ removed the legacy Clutter click/tap action classes. GNOME 50 moved
            ...
            // GNOME 45–48: use legacy button-press-event / scroll-event signals.
    ...
    #addMouseButtonGesture(actor, mouseButton, callback) {
        ...
        if (typeof gesture.set_required_button === "function") gesture.set_required_button(mouseButton);
        if (typeof gesture.set_recognize_on_press === "function") gesture.set_recognize_on_press(true);
