...
function createAlbumArtCacheKey(albumArtUri) {
    return GLib.compute_checksum_for_string(GLib.ChecksumType.SHA256, albumArtUri, -1);
}

function createAlbumArtCacheFile(albumArtUri) {
    return Gio.File.new_for_path(GLib.build_filenamev([albumArtCacheDirectoryPath, createAlbumArtCacheKey(albumArtUri)]));
}
...
export default class AlbumArtLoader {
    ...
    #getSession() {
        this.#session ??= new Soup.Session({ timeout: ALBUM_ART_REQUEST_TIMEOUT_SECONDS });
        return this.#session;
    }

    #getCacheWriteCancellable() {
        if (!this.#cacheWriteCancellable || this.#cacheWriteCancellable.is_cancelled())
    ...
    #getAlbumArtCacheFile(albumArtUri) {
        return this.#ensureAlbumArtCacheDirectory() ? createAlbumArtCacheFile(albumArtUri) : null;
    }
    ...
    #writeAlbumArtCacheBytes(cacheFile, responseBytes, cacheKey) {
        ...
        cacheFile
            ...
            .catch((error) => {
                if (!isCancellationError(error)) logger.warn("Failed to persist album art in the cache", error);
            });
    }
    ...
    async #readRemoteAlbumArtBytes(httpMessage, loadCancellable) {
        ...
        try {
            ...
            responseStream = await this.#getSession().send_async(httpMessage, GLib.PRIORITY_DEFAULT, loadCancellable);
    ...
    async loadAlbumArt(albumArtUri, isCacheEnabled, loadCancellable, { bypassCacheRead = false } = {}) {
        ...
        if (scheme === "file") {
            const stream = await this.#openAlbumArtInputStream(Gio.File.new_for_uri(uri.to_string()), loadCancellable);
        ...
        const cacheKey = isCacheEnabled ? createAlbumArtCacheKey(albumArtUri) : null;
        const cacheFile = isCacheEnabled ? this.#getAlbumArtCacheFile(albumArtUri) : null;
        const cachedStream = bypassCacheRead
            ? null
            : await this.#openCachedAlbumArtInputStream(cacheFile, loadCancellable, cacheKey);
        if (cachedStream) {
        ...
        const responseBytes = await this.#readRemoteAlbumArtBytes(httpMessage, loadCancellable);
