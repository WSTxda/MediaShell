...
export default class MprisProxyFactory {
  ...
  async init() {
    ...
    const introspectionDataPromise = loadMprisIntrospectionData(
      initializationCancellable,
    );
    ...
    try {
      ...
      if (this.initializationCancellable === initializationCancellable)
        this.initializationCancellable = null;
      if (this.introspectionDataPromise === introspectionDataPromise)
        this.introspectionDataPromise = null;
    }
  }
  ...
  createProxy(interfaceInfo, busName, objectPath, flags, cancellable = null) {
    if (!interfaceInfo)
      throw new Error(`D-Bus interface is unavailable for ${busName}`);
