...
import {
    isMediaAppBlocked,
    isShellAppStopped,
    resolveLifecycleShellApp,
} from "../services/MediaAppResolver.js";
import PlayerProxy from "./PlayerProxy.js";
import { selectActiveMediaApp, selectNextMediaApp } from "./MediaAppSelectionPolicy.js";

Gio._promisify(Gio.DBusProxy.prototype, "call", "call_finish");

const logger = createLogger("MediaAppRegistry");
const MEDIA_APP_DISAPPEARANCE_GRACE_MILLISECONDS = 5000;
...
export default class MediaAppRegistry {
    ...
    async init() {
        ...
        const dbusProxy = await this.mprisProxyFactory.createBusProxy(this.operationCancellable);
        if (this.destroyed || lifecycleGeneration !== this.lifecycleGeneration) return;
        this.dbusProxy = dbusProxy;

        this.dbusOwnerSignalId = this.dbusProxy.connect("notify::g-name-owner", () => this.handleBusOwnerChanged());
    ...
    handleBusOwnerChanged() {
        if (this.destroyed || !this.dbusProxy) return;

        if (!this.dbusProxy.get_name_owner()) {
            for (const busName of this.mediaAppProxies.keys()) this.scheduleMediaAppRemoval(busName);
            return;
        }

        this.discoverRunningMediaApps().catch((error) => {
            if (!isCancellationError(error)) logger.warn("Failed to rediscover MPRIS apps after D-Bus recovery", error);
        });
    }
    ...
    reconcileMediaAppOwner(busName) {
        ...
        if (!mediaApp) {
            this.registerMediaApp(busName).catch((error) => {
                if (!isCancellationError(error))
                    logger.warnOnce(`register:${busName}`, "Failed to add MPRIS app", busName, error);
            });
            return;
        }
        ...
        mediaApp.refreshMetadata().catch((error) => {
            if (!isCancellationError(error))
                logger.debug("Metadata refresh failed after MPRIS owner recovery", busName, error);
        });
    ...
    async discoverRunningMediaApps() {
        if (!this.dbusProxy || this.destroyed) return;
        ...
        const mediaAppBusNames = busNames.filter((busName) => busName.startsWith(MPRIS_PREFIX));
    ...
    async registerMediaApp(busName) {
        if (this.destroyed || this.mediaAppProxies.has(busName) || this.pendingMediaAppProxies.has(busName)) return;
        ...
        try {
            ...
            if (
                ...
                logger.debug("Ignored blocked app", busName);
                mediaAppProxy.destroy();
                return;
            }

            mediaAppProxy.onPropertyChanged("IsPinned", () => this.selectActiveMediaApp());
            mediaAppProxy.onPropertyChanged("PlaybackStatus", () => this.selectActiveMediaApp());
            ...
            if (this.pendingMediaAppProxies.get(busName) === mediaAppProxy) this.pendingMediaAppProxies.delete(busName);
            if (!adopted && !mediaAppProxy.destroyed) mediaAppProxy.destroy();
        }
    }
    ...
    scheduleMediaAppRemoval(busName) {
        ...
        const sourceId = GLib.timeout_add(GLib.PRIORITY_DEFAULT, MEDIA_APP_DISAPPEARANCE_GRACE_MILLISECONDS, () => {
            this.pendingRemovalSourceIds.delete(busName);
            const mediaApp = this.mediaAppProxies.get(busName);
            if (mediaApp?.hasBusOwner) this.reconcileMediaAppOwner(busName);
            else this.unregisterMediaApp(busName);
            return GLib.SOURCE_REMOVE;
        });
        this.pendingRemovalSourceIds.set(busName, sourceId);
        logger.debug("Deferred ownerless MPRIS app removal for a possible replacement owner", busName);
    }

    observeExactMediaAppShutdown(busName, mediaApp) {
        const shellApp = resolveLifecycleShellApp(mediaApp.desktopEntry);
        const removeIfStopped = () => {
            if (!this.pendingRemovalBusNames.has(busName) || !isShellAppStopped(shellApp)) return false;

            logger.debug("Removed ownerless MPRIS app after its exact desktop app stopped", busName);
            this.unregisterMediaApp(busName);
            return true;
        };
        ...
        try {
            ...
            this.pendingRemovalAppStateConnections.set(busName, { shellApp, stateSignalId });

            // Close the race between the initial state read and signal
            // connection without introducing polling.
            return removeIfStopped();
        } catch (error) {
            logger.debug("Could not observe exact Shell app state during MPRIS hand-off", busName, error);
            return false;
        }
    }
    ...
    cancelScheduledRemoval(busName) {
        ...
        if (stateSignal) {
            try {
                ...
                logger.debug("Shell app state signal was already disconnected", busName, error);
            }
        ...
        if (cancelled) logger.debug("Cancelled deferred MPRIS app removal", busName);
        return cancelled;
    }
    ...
    refreshVisibleMediaApps(forceNotification = false) {
        const nextVisibleMediaApps = [...this.mediaAppProxies.values()].filter(
            (mediaAppProxy) =>
                !mediaAppProxy.isMediaAppInvalid && !this.pendingRemovalBusNames.has(mediaAppProxy.busName),
        );
        const listChanged =
            nextVisibleMediaApps.length !== this.visibleMediaApps.length ||
            nextVisibleMediaApps.some((mediaApp, index) => mediaApp !== this.visibleMediaApps[index]);
        if (!listChanged && !forceNotification) return false;

        this.visibleMediaApps = nextVisibleMediaApps;
        this.invokeCallbackSafely(this.onMediaAppsChanged, this.visibleMediaApps, "media-apps-changed");
        return true;
    }
    ...
    getPinnedMediaApp() {
        return [...this.mediaAppProxies.values()].find((mediaApp) => mediaApp.isAppPinned()) ?? null;
    }
    ...
    activateMediaApp(mediaApp) {
        ...
        this.invokeCallbackSafely(this.onActiveMediaAppChanged, mediaApp, "active-media-app-changed");
        return true;
    }
    ...
    activateNextMediaApp() {
        ...
        const nextMediaApp = selectNextMediaApp(this.getMediaApps(), this.activeMediaApp);
    ...
    toggleMediaAppPin(mediaApp) {
        ...
        return mediaApp.isAppPinned() ? this.unpinMediaApp(mediaApp) : this.pinMediaApp(mediaApp);
    }
    ...
    selectActiveMediaApp() {
        ...
        this.invokeCallbackSafely(this.onActiveMediaAppChanged, selected, "active-media-app-changed");
    }
    ...
    invokeCallbackSafely(callback, value, eventName) {
        try {
            ...
            logger.errorOnce(`callback:${eventName}`, `Registry callback failed: ${eventName}`, error);
        }
    }
    ...
    async setBlockedAppIds(blockedAppIds) {
        ...
        try {
            ...
            if (!isCancellationError(error)) logger.warn("Failed to refresh media apps after blocked-app change", error);
        }
    }
    ...
    destroy() {
        ...
        for (const busName of [...this.pendingRemovalBusNames]) this.cancelScheduledRemoval(busName);
        this.pendingRemovalSourceIds.clear();
        this.pendingRemovalAppStateConnections.clear();
        this.pendingRemovalBusNames.clear();

        for (const mediaAppProxy of this.mediaAppProxies.values()) mediaAppProxy.destroy();
        for (const mediaAppProxy of this.pendingMediaAppProxies.values()) mediaAppProxy.destroy();
