...
export default class SettingsStore {
    ...
    readSettingIntoTarget(key, spec) {
        ...
        try {
            ...
            logger.warn(`Failed to normalize setting ${key}; using a safe default`, error);
            value = this.readFallbackValue(key, spec);
            if (spec.transform) value = spec.transform(value);
        }
    ...
    readFallbackValue(key, spec) {
        ...
        try {
            const schemaDefault = this.settings.get_default_value(key)?.recursiveUnpack?.();
            if (schemaDefault !== undefined) return schemaDefault;
        } catch (error) {
            logger.debugOnce(`schema-default:${key}`, `Schema default for ${key} could not be read`, error);
        }
    ...
    destroy() {
        ...
        for (const signalId of this.settingChangeSignalIds) {
            try {
                ...
                logger.debug("A settings signal was already disconnected during teardown", error);
            }
        }
