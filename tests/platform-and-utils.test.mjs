...
test("duration formatting handles minutes and hours", () => {
  ...
  assert.equal(enumValueByIndex({ FIRST: "first", SECOND: "second" }, 1), "second");
});

test("string-list normalization removes empty and duplicate persisted values", () => {
  assert.deepEqual(normalizeUniqueStrings(["app.desktop", "", " app.desktop ", null, "other.desktop"]), [
    "app.desktop",
    "other.desktop",
  ]);
...
test("ordered setting values drop corruption, remove duplicates, and restore missing entries", () => {
  const allowed = ["APP_ICON", "TRACK_INFORMATION", "VISUALIZER", "PLAYBACK_CONTROLS"];
  assert.deepEqual(
    normalizeOrderedValues(["PLAYBACK_CONTROLS", "UNKNOWN", "APP_ICON", "APP_ICON"], allowed),
...
test("MPRIS artist metadata is normalized without leaking invalid values", () => {
  assert.equal(formatArtistNames(["Artist A", "Artist B"]), "Artist A, Artist B");
...
test("app identity normalization removes desktop suffixes and unstable bus segments", () => {
  assert.equal(stripDesktopFileSuffix("org.mozilla.firefox.desktop"), "org.mozilla.firefox");
  assert.equal(normalizeAppIdentity("  Música—Player.desktop "), "musica player");
  ...
  const appIds = buildDesktopAppIdCandidates("Firefox", "org.mozilla.firefox.desktop");
...
test("installed-app search is case, accent, punctuation, and desktop-ID tolerant", () => {
  assert.equal(normalizeSearchText("  Google.Chrome—Beta.desktop  "), "google chrome beta desktop");
  assert.equal(matchesSearchText("chrome", ["Google Chrome", "com.google.Chrome.desktop"]), true);
  assert.equal(matchesSearchText("música", ["Musica Player", "org.example.Player.desktop"]), true);
  assert.equal(matchesSearchText("com google chrome", ["Google Chrome", "com.google.Chrome.desktop"]), true);
  assert.equal(matchesSearchText("chrome google", ["Google Chrome", "com.google.Chrome.desktop"]), true);
  assert.equal(matchesSearchText("firefox", ["Google Chrome", "com.google.Chrome.desktop"]), false);

  const searchIndex = buildSearchIndex(["Google Chrome", "com.google.Chrome.desktop"]);
  assert.equal(searchIndex, "google chrome com google chrome desktop");
  assert.deepEqual(tokenizeSearchQuery("  Chrome—Google  "), ["chrome", "google"]);
...
test("visualizer styles produce bounded fixed-size levels", () => {
  ...
  for (const style of [VisualizerStyles.WAVE, VisualizerStyles.PULSE]) {
    ...
    assert.equal(getVisualizerBarLevels(style, 0.5, 4, reusableFrame), reusableFrame);
...
test("MPRIS values are normalized to specification-safe defaults", async () => {
  ...
  assert.equal(metadataContainsTrack({ "mpris:trackid": MPRIS_NO_TRACK_PATH, "xesam:title": "Stale" }), false);
